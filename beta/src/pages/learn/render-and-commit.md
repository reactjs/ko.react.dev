...
컴포넌트가 처음 렌더링되면 [`setState`](reference/setstate)로 상태를 업데이트하여 추가 렌더링을 트리거 시킬 수 있습니다. 컴포넌트의 상태를 업데이트하면 렌더링이 자동으로 대기열에 추가됩니다. (이것은 식당에서 손님이 첫 주문 이후에 갈증이나 배고픔에 따라 차, 디저트 등을 추가로 주문하는 모습으로 상상해 볼 수 있습니다.)
...
* **이후 렌더링에서** React는 상태 업데이트가 일어나 렌더링을 트리거한 컴포넌트를 호출합니다.
...
* **이전의 상태를 변경해서는 안됩니다.** 렌더링 전에 존재했던 객체나 변수를 변경해서는 안 됩니다. (누군가의 주문이 다른 사람의 주문을 변경해서는 안 됩니다.)
