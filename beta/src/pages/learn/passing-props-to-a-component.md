...
과학자 중 한 명이 사용하는 마크업을 추출해서 보는 것으로 시작하세요. 두 번째 예시와 일치하지 않는 부분들을 찾아, props 별로 알맞게 설정 가능하도록 바꾸세요.
...
`imageSize` prop은 기본값을 가지고 있어서, 컴포넌트에 prop을 전달하지 않는다는 것을 기억하세요.
...
`award`가 배열인 경우 추가적인 `awardCount`prop이 필요하지 않는다는 것을 기억하세요. `awards.length`를 사용하여 수상 횟수를 셀 수 있습니다. props는 어떤 값이라도 전달할 수 있으며, 배열도 포함된다는 것을 기억하세요!

이 페이지의 이전 예와 더 유사한 또 다른 해결책은, 단일 객체에 있는 person 안에 있는 모든 정보를 그룹화하고, 해당 객체의 그룹화된 정보를 하나의 prop을 사용하여 전달하는 것이 있습니다.
...
JSX 속성의 집합이 아니라 JavaScript 개체의 속성을 설명하므로 문법이 약간 다르게 사용된 것처럼 보이지만, 이러한 예제는 대부분 동일하며, 어느 하나의 접근 방식을 선택할 수 있습니다.

</Solution>

### prop에 기반하여 이미지를 조정하기 {/*adjust-the-image-size-based-on-a-prop*/}

이 예제에서, `Avatar`는 `<img>`의 넓이, 높이를 결정하는 숫자 형태의 `size` prop을 받습니다. 이 예제에서 `size` prop은 `40`으로 설정되어 있습니다. 그러나, 새 탭에서 이미지를 연다면, 이미지가 더 크다는 것을 알 수 있니다(`160`픽셀). 실제 이미지 크기는 당신이 요청한 사이즈 그대로 결정됩니다. 

`Avatar` 컴포넌트를 바꿀 때 `size` prop에 기반하여 요청한 가장 유사한 이미지 사이즈로 요청하세요. 특히, `size`가 `90`보다 작으면 `'b'` ("big")보다는 `getImageUrl` 함수로 `'s'` ("small")를 전달합니다. `size` prop의 값이 다른 아바타를 렌더링 하고, 새 탭에서 이미지를 열어 변경 사항이 작동하는지 확인합니다.
