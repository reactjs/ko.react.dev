...
Hooks가 현재 존재하고 있는 모든 클래스 사용 사례을 다룰 수 있도록 할 생각이지만, **미래에도 계속 클래스 컴포넌트들을 지원할 예정입니다.** 페이스북에서 수만 개의 클래스 컴포넌트들을 작성했으며, 그들을 재작성할 계획이 전혀 없습니다. 대신에, 새로운 코드에서 기존 코드와 나란히 Hooks를 사용할 계획입니다.
