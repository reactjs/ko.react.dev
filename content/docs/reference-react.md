...
React 컴포넌트를 사용하면 UI를 독립적이고 재사용할 수 있는 부분들로 나누어서 각 부분들을 격리시킬 수 있습니다. React 컴포넌트들은 `React.Component` 또는 `React.PureComponent`로 세부적으로 나누어 정의할 수 있습니다.
...
### React 요소 생성하기 {#creating-react-elements}

UI의 형태를 설명하는 데에 [JSX를 사용할 것](/docs/introducing-jsx.html)을 권장합니다. 각 JSX 요소는 단지 [`React.createElement()`](#createelement)를 호출하는 Syntactic Sugar에 불과합니다. JSX를 사용할 경우 아래의 메서드들을 직접 호출하는 일은 거의 없습니다.
...
`React.Component` 기초 클래스와 관련된 메서드와 속성 목록은 [React.Component API Reference](/docs/react-component.html)에서 확인할 수 있습니다.
...
> 주의
>
> `React.PureComponent`의 `shouldComponentUpdate()`는 컴포넌트에 대하여 얕은 비교만을 수행합니다. 따라서 컴포넌트에 복잡한 자료 구조가 포함되어있다면, 깊은 차이가 존재함에도 불구하고 차이가 없다고 판단하는 잘못된 결과를 만들어낼 수 있습니다. props와 상태의 구조가 간단할 것으로 예상될 때에만 `PureComponent`를 상속하고, 깊은 자료 구조의 변화가 있다면 [`forceUpdate()`](/docs/react-component.html#forceupdate)를 사용하세요. 또는 중첩된 데이터들을 빠르게 비교할 수 있도록 하려면 [불변 객체](https://facebook.github.io/immutable-js/)의 사용을 검토해보세요.
>
...
`React.memo`는 [고차 컴포넌트](/docs/higher-order-components.html)입니다. [`React.PureComponent`](#reactpurecomponent)와 유사하지만 클래스가 아니라 함수형 컴포넌트라는 점이 다릅니다.
...
이 메서드는 오직 **[성능 최적화](/docs/optimizing-performance.html)**를 위하여 사용됩니다. 렌더링 "방지"에 사용하지 마세요. 버그를 만들 수 있습니다.

> 주의
>
> 클래스 컴포넌트의 [`shouldComponentUpdate()`](/docs/react-component.html#shouldcomponentupdate) 메서드와 달리,  method on class components, `areEqual` 함수는 props들이 서로 같으면 `true`를 반환하고, props들이 서로 다르면 `false`를 반환합니다. 이것은 `shouldComponentUpdate`와 정반대의 동작입니다.
...
인자로 주어지는 타입에 따라 새로운 [React 요소](/docs/rendering-elements.html)를 생성하여 반환합니다. type 인자로는 태그 이름 문자열(`'div'` 또는 `'span'` 등), [React 컴포넌트](/docs/components-and-props.html) 타입, 또는 [React Fragment](#reactfragment) 타입 중 하나가 올 수 있습니다.
...
`element`를 기준으로 새로운 React 요소를 복사하고 반환합니다. 새로운 요소에는 원본 요소가 가졌던 props가 새로운 props와 얕게 합쳐진 뒤 주어집니다. 새로운 자식들은 기존의 자식들로 대체합니다. 원본 요소의 `key`와 `ref`는 그대로 유지됩니다.
...
그렇지만 `ref`들이 유지된다는 점이 다릅니다. 즉 조상이 가지고 있을 `ref`를 사용하여 자식 요소에 접근하는 것이 허용됩니다. 새로운 요소에 덧붙여지는 것과 동일한 `ref`를 얻을 수 있습니다.
...
이 헬퍼 함수는 레거시 기능으로 간주되며, JSX 또는 `React.createElement()`를 사용할 것을 권장합니다.
...
객체가 React 요소인지 확인합니다. `true` 또는 `false`를 반환합니다.
...
`children`에 포함된 각 요소들에 대하여 `this`를 `thisArg`의 값으로 설정한 함수를 호출합니다. `children`이 배열일 경우, 이 배열의 각 요소를 순회하면서 각 요소에 대하여 함수가 호출됩니다. `children`이 `null` 또는 `undefined`일 경우, 이 메서드는 배열이 아니라 `null` 또는 `undefined`를 반환합니다.

> 주의
>
> `children`이 `Fragment`일 경우, `children`은 단일 요소로 취급되어 순회하지 않습니다.
...
[`React.Children.map()`](#reactchildrenmap)와 동일하지만, 배열을 반환하지 않습니다.
...
`children`이 단 하나의 자식(React 요소)를 갖는지 확인하고 해당 자식 요소를 반환합니다. 그렇지 않을 경우 오류를 발생시킵니다.

> 주의
>
> `React.Children.only()`는 [`React.Children.map()`](#reactchildrenmap)의 반환값을 허용하지 않는데, 왜냐하면 반환값이 React 요소가 아니라 배열이기 때문입니다.
...
`children` 불투명(Opaque) 자료 구조의 각 요소에 키(Key)를 부여하여 평평한 배열로 만든 뒤 반환합니다. `render()` 메서드에서 `children`의 집합을 다루고 싶을 때, 특히 `this.props.children`을 하부로 전달하기 전에 다시 정렬하거나 일부만 잘라내고 싶을 때에 유용합니다.

> 주의
>
> `React.Children.toArray()`는 `children`을 평평하게 만들 때, 중첩된 배열들의 의미를 보존하기 위하여 키를 변경합니다. 즉, `toArray`는 반환되는 배열에 키값을 덧붙여서 각 요소가 갖는 키가 평평해진 배열 내에서만 유효한 범위를 형성하도록 해줍니다.
...
`React.createRef`는 React 요소에 `ref` 특성으로 붙일 수 있는 [ref](/docs/refs-and-the-dom.html)를 생성합니다.
...
위의 예시에서 React는 `<FancyButton ref={ref}>` 요소에 주어진 `ref`를 `React.forwardRef` 호출시 렌더링 함수에 2번째 인자로 전달합니다. 이 렌더링 함수는 `ref`를 `<button ref={ref}>` 요소에 전달합니다.

따라서 React가 해당 ref를 붙이고 난 뒤, `ref.current`는 `<button>` DOM 요소를 직접 가리키게 됩니다.
...
사용 방법은 [코드 스플리팅](/docs/code-splitting.html#reactlazy) 문서에서 익힐 수 있습니다. 또한 사용 방법을 자세히 다룬 [이 글](https://medium.com/@pomber/lazy-loading-and-preloading-components-in-react-16-6-804de091c82d)을 읽어보는 것도 좋습니다.
...
관련된 내용을 [코드 스플리팅 가이드](/docs/code-splitting.html#reactlazy) 문서에서 설명하고 있습니다. `lazy`한 컴포넌트는 `Suspense` 트리 내의 깊숙한 곳에 위치할 수 있다는 점에 유의하세요. 즉, `Suspense`가 모든 컴포넌트를 감쌀 필요는 없다는 것입니다. 가장 좋은 사용법은 로딩 지시기를 사용하고 싶은 지점에 `<Suspense>`를 작성하는 것이지만, 코드 스플리팅을 하고자 하는 지점 어디서든지 `lazy()`를 써야 할 것입니다.
