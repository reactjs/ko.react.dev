...
* `precedence` 속성을 사용할 경우, React는 `href`와 `precedence`를 제외한 다른 모든 props를 적용하지 않습니다.
...
`precedence` 속성을 제공하면 React는 컴포넌트 트리에서 해당 값이 표시되는 순서에 따라 인라인 스타일시트의 순서를 다시 지정합니다.

인라인 스타일시트는 로딩 중이더라도 Suspense 경계를 트리거하지 않습니다.
글꼴이나 이미지와 같은 비동기 리소스를 로드하는 경우에도 마찬가지입니다.
